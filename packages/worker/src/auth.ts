import { API_KEY_PREFIX, type AuthenticateInput, type AuthenticateResult, type PermissionEntry, type PrincipalType } from '@propustka/core'
...
//
// The successor to `resolveRequest` for the management RPCs (`issueKey`/`issueJwt`/`revokeKey`/
// `listPrincipals`): resolve the CALLER from a propustka-native credential the SDK forwards — a
// `px_token` access JWT (verified against our OWN signing keys) or an opaque `px_` key (resolved via
// the same `resolveCredential` core as `mintFromKey`). There is no Cloudflare Access JWT anymore.
...
export async function resolveCaller(
	...
	// LOCAL DEV BYPASS (mirrors resolveRequest). With NO durable signing keys configured (an ephemeral
	// key ⇒ dev-only — the new "no real auth configured" signal that ACCESS_APPS-empty used to be) AND
	// no credential presented, resolve a fixed global-admin so the example app / admin scripts work
